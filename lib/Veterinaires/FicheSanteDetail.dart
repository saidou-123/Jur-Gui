...
class _FicheSanteDetailState extends State<FicheSanteDetail>
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(
              text: 'Consultations (${_consultations.length})',
  ...
  Widget _buildInfoBanner() => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(
        ...
        children: [
          Text(
            widget.animal['nom'] ?? 'N/A',
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
          ),
          Text(
            '${widget.animal['race'] ?? 'N/A'} · ${widget.animal['sexe'] ?? 'N/A'}',
  ...
  Widget _mini(String val, IconData icon, Color c) =>
  ...
  Widget _buildConsultations() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) =>
            ConsultationItem(consultation: _consultations[i]),
      ),
    );
  }
  ...
  Widget _buildVaccinations() {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) =>
            VaccinationItem(vaccination: _vaccinations[i]),
      ),
    );
  }
  ...
  Future<void> _ajouterConsultation() async {
    final ok = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => NouvelleConsultationPage(
  ...
  Future<void> _ajouterVaccination() async {
    final ok = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => NouvelleVaccinationForm(
          animal: widget.animal,
          source: widget.source,
        ),
      ),
