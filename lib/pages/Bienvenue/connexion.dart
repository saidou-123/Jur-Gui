...
class _ConnexionState extends State<Connexion> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      if (userData == null) {
        ...
        _navigateByRole(role, null);
      } else {
        debugPrint('✅ Rôle récupéré: ${userData['role']}');
        _navigateByRole(userData['role'] as String, userData['statut'] as String?);
      }
  ...
  // ===== NAVIGATION PAR RÔLE (avec vérification du statut vétérinaire) =====
  void _navigateByRole(String role, String? statut) {
    if (!mounted) return;

    debugPrint('🚀 Navigation vers interface: $role (statut: $statut)');

    Widget destination;

    if (role.toLowerCase() == 'veterinaire') {
      // Règle : statut == null -> ancien compte créé avant la mise en place
      // de la vérification, on ne le bloque pas rétroactivement.
      // statut == 'approved' -> accès normal.
      // statut == 'pending_verification' -> page d'attente.
      // statut == 'rejected' -> page de refus.
      if (statut == 'pending_verification') {
        destination = const VetPendingPage();
      } else if (statut == 'rejected') {
        destination = const VetRejectedPage();
      } else {
        // null (ancien compte) ou 'approved'
        destination = const interfaceVeterinaire();
      }
    } else if (role.toLowerCase() == 'eleveur') {
